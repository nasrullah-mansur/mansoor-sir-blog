...
class MainCourseDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', 'maincourse.action')
            ->addIndexColumn()
            ...
            ->editColumn('status', function ($data) {
                ...
                elseif($data->status == STATUS_UPCOMING) {
                    return '<span class="text-info p-1">Upcoming</span>';
                }
            ...
            ->editColumn('action', function ($data) {
                return
                    '<div class="d-flex action-btn">
                        <a class="btn btn-icon btn-success" style="margin-right: 5px;" href="' . route('main.course.edit', $data->id) . '"><i class="ft-edit"></i></a>
                        <a data-id="' . $data->id . '" class="btn btn-icon btn-danger delete-data" style="margin-right: 5px;" href="#"><i class="ft-trash-2"></i></a> 
                    </div>';
            })
            ->rawColumns(['action', 'status', 'image', 'title', 'slug']);
    }

    /**
     * Get query source of dataTable.
     *
     * @param \App\Models\MainCourse $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(MainCourse $model): QueryBuilder
    {
    ...
    {
        return 'MainCourse_' . date('YmdHis');
    }
}
