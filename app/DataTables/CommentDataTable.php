    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->editColumn('status', function ($data) {
            if ($data->status == STATUS_ACTIVE) {
                return '<a title="Click to change status" href="' . route('comment.status', $data->id) .  '"><span class="text-primary p-1">Public</span></a>';
            } else {
                return '<a title="Click to change status" href="' . route('comment.status', $data->id) .  '"><span class="text-warning p-1">Draft</span></a>';
            }
        })
