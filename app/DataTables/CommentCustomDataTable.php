...
class CommentCustomDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->editColumn('status', function ($data) {
            if ($data->status == STATUS_ACTIVE) {
                return '<a title="Click to change status" href="' . route('comment.custom.status', $data->id) .  '"><span class="text-primary p-1">Public</span></a>';
            } else {
                return '<a title="Click to change status" href="' . route('comment.custom.status', $data->id) .  '"><span class="text-warning p-1">Draft</span></a>';
            }
        })
     ...
     *
     * @param \App\Models\CommentCustom $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(CommentCustom $model): QueryBuilder
    {
    ...
    {
        return 'CommentCustom_' . date('YmdHis');
    }
}
